...
//         socket.on('connect', () => ToastAndroid.show('Driver connected', ToastAndroid.SHORT));
...
const SOCKET_URL = process.env.API_BASE_URL || "http://192.168.1.68:8080";
...
export const initSocket = (): Socket => {
  if (!socket) {
    socket = io(SOCKET_URL, { transports: ["websocket"], forceNew: true });

    socket.on("connect", () => {
      if (Platform.OS === "android") {
        ToastAndroid.show("Connected via socket", ToastAndroid.SHORT);
      }
...
export const onMessageReceived = (
  cb: (msg: { senderId: string; text?: string; chatFile?: string | null }) => void
): void => {
  socket?.on("getMessage", cb);
};
...
export const onUsersUpdate = (
  cb: (users: { userId: string; socketId: string }[]) => void
): void => {
  socket?.on("getUsers", cb);
};
